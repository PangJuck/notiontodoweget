...
| | 보는 것 | 고치는 것 |
|---|---|---|
| 관리자 (admins에 적힌 사람) | 업무 · 개인 · 팀 전체 (남의 비공개 제외) | 전부 (남의 비공개 제외) |
...
전원 공개가 기본이지만, 개인 사정처럼 팀에 열 일이 아닌 것도 있다. 팀 DB의
`비공개` 체크박스를 켜면 **주인 말고 아무에게도 안 보인다 — 관리자도 마찬가지다.**

- 거르는 자리는 **노션 질의**다. 남의 비공개 항목은 워커 메모리에도 올라오지
  않으니 브라우저로 샐 길이 없다
- 고치는 것도 막힌다. 관리자가 id를 알고 있어도 거부된다. 보이지도 않는 것을
  id만으로 고칠 수 있으면 가리는 의미가 없다
- 화면에서는 항목의 `🔒`, 새로 넣을 때는 `나만 보기` 체크. **넣을 때 켜는 쪽이
  낫다** — 만든 뒤에 잠그면 그 사이에 한 번 팀에 보인다
- 팀원의 Claude에서는 `add_todo`의 `private`, 또는 `set_private`

**노션 자체는 성준에게 열려 있다.** 팀 DB가 성준 개인 워크스페이스의 DB이기
때문이다. 웹판과 MCP에서 가려질 뿐이고, 노션 앱을 직접 열면 보인다. 팀원에게
안내할 때 이 선을 넘겨 말하지 않는다 — "동료에게 안 보인다"는 맞고,
"아무도 못 본다"는 틀리다.
...
보는 것은 하나다 — 남의 것이 새어 나가지 않는가. 거르개가 노션 질의에 실려
나가는지, 거부된 요청이 노션에 쓰기를 시도조차 안 하는지까지 본다. 신원·권한
쪽을 고쳤으면 배포 전에 돌린다.

## 캘린더 연동 (아직 안 붙임)

`worker/connectors/calendar.js`는 자리만 잡아 둔 것이다. `CALENDAR` 시크릿이
없으면 아무 일도 하지 않는다. 구글 캘린더를 붙일 때 무엇을 정해야 하는지는
