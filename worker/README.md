...
`worker/connectors/calendar.js`는 자리만 잡아 둔 것이다. `CALENDAR` 시크릿이
없으면 아무 일도 하지 않는다. 팀원 캘린더에 쓰거나 회의실을 잡으려면 구글
캘린더 API가 필요한데, 무엇을 정해야 하는지는
`docs/구글-캘린더-연동-계획.md`에 적어 뒀다.
