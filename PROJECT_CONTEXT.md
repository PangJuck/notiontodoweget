...
| `/sync/<토큰>` | **cron-job.org가 10분마다** 두드린다 | 2026-09-16 등록 |
...
**밖에서 두드리는 것은 cron-job.org다.** `ulick-todo sync`라는 작업 하나가
10분마다(`*/10 * * * *`, Asia/Seoul) 저 주소를 부른다. 성준의 cron-job.org
계정에 있고, 3회 연속 실패하면 메일이 온다. 주소에 토큰이 박혀 있으므로
그 작업을 남에게 보여주지 않는다.

돌아오는 본문이 `ok`가 아니라 `off`면 **`CALENDAR` 시크릿을 못 읽은 것**이다.
둘 다 200이라 상태 코드로는 구분이 안 되므로, cron-job.org의 "Save responses
in job history"를 켜 뒀다. 그게 아니면 아무 일도 안 하면서 초록불만 계속 뜬다.
