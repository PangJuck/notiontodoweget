...
|---|---|---|---|
| 업무 | `0e928040351d4fdfae49f77e67e914e6` | `5ead04e6-65a7-4e98-9b3c-a8dfffdf5001` | `📎 오늘의 할일은 뭘까요 (피피카츄)constant` 페이지 안 |
| 개인 | `1730d225784340f88e15f9af9d51ea78` | `bd927ce1-075b-4b3a-b98a-7c823cbb6cfd` | **위와 같은 페이지 안** |
| 팀 | `6f9008aa63f249109b6ed29a374b529d` | `4045c7f7-673a-4f36-a60e-6c6e0fd05b02` | 최상위 비공개 DB (팀원에게 공유 안 함) |

셋 다 **성준 개인 노션(`ulick sungjune의 Notion`, ghaos009@gmail.com)** 안에 있다.
이 워크스페이스의 사람은 성준 혼자이고, 나머지는 봇 둘(노션 MCP, `notion_todo-widget`
Integration)뿐이다. 회사 노션이 따로 끼어 있지 않다.

**업무 DB와 개인 DB는 한 페이지에 나란히 있다.** 그래서 그 페이지를 웹에
게시하거나 링크로 공유하면 **개인 DB도 같이 열린다.** 노션 사이드바 기준으로 그
페이지는 `Private`에 있지만, 웹 게시(공개 링크) 여부는 API로 확인되지 않는다 —
개인 항목이 새면 안 되는 상황이면 그 페이지의 공유 설정을 사람이 직접 확인하고,
필요하면 개인 DB를 다른 비공개 페이지로 옮긴다(DB id는 그대로라 코드는 안 고쳐도
된다. 옮긴 뒤 `notion_todo-widget` 연결만 다시 확인한다).
...
개인 DB로. 회사 업무는 업무 DB로. 팀과 나눌 일만 팀 DB로 — 팀 DB에 올라간 것은
팀원끼리 서로 다 본다.
