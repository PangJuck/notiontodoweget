...
|---|---|---|---|
| 팀 | `6f9008aa63f249109b6ed29a374b529d` | `4045c7f7-673a-4f36-a60e-6c6e0fd05b02` | 최상위 비공개 DB (팀원에게 공유 안 함) |
| 개인 | `1730d225784340f88e15f9af9d51ea78` | `bd927ce1-075b-4b3a-b98a-7c823cbb6cfd` | `📎 오늘의 할일은 뭘까요 (피피카츄)constant` 페이지 안 |
...
`담당자: 성준`으로 옮겼고, 코드는 이제 그 DB를 **읽지도 쓰지도 않는다.** 노션에는
아직 남아 있으니 성준이 확인하고 지우면 된다.
...
개인 DB는 옛 업무 DB와 **같은 페이지 안**에 있다. 그 페이지를 웹에 게시하거나
링크로 공유하면 **개인 DB도 같이 열린다.** 노션 사이드바 기준으로 그 페이지는
`Private`에 있지만, 웹 게시(공개 링크) 여부는 API로 확인되지 않는다 — 개인 항목이
새면 안 되는 상황이면 그 페이지의 공유 설정을 사람이 직접 확인하고, 필요하면 개인
DB를 다른 비공개 페이지로 옮긴다(DB id는 그대로라 코드는 안 고쳐도 된다. 옮긴 뒤
`notion_todo-widget` 연결만 다시 확인한다).
