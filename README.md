...
**DB는 둘이다 — 팀과 개인.** 회사 일은 성준 혼자 하는 것까지 전부 팀 DB로 가고
(`담당자`로 구분한다), 회사와 무관한 것만 개인 DB로 간다. 셋이던 시절의 `업무`
DB는 없앴다. 어디로 들어갈지 헷갈리는 것이 목록이 하나 더 있어서 얻는 것보다
비쌌다.
...
| **기록** | 완료분을 일별·주별·월별로. 기본 3개월, 더 옛날은 눌러서 |
...
그 아래 **전체 / 개인 / 팀** 줄로 보기 범위를 바꾼다(팀 모드가 꺼져 있으면 개인
하나뿐이라 줄 자체가 안 뜬다). 레이아웃은 그대로다.

- **추가할 때 어디로 가는지 버튼에 적힌다** (`팀에 추가`). 전체를 보는 중이면
  아무것도 골라 두지 않는다 — 고르기 전에는 넣지 않는다

- 항목에 마우스를 올리면 `★` 오늘의 3, `1~4` 사분면 이동, `⏳` 대기중, `📅` 마감일,
  `✎` 메모, `×` 삭제
- **마감일은 캘린더에서 끌어다 옮긴다.** 칸에서 칸으로 끌면 노션의 마감일이 그날로
  바뀐다(매트릭스의 순서 드래그와 다르다 — 그쪽은 이 브라우저에만 기억한다).
  손가락으로는 끌기가 안 잡히므로 `📅` 버튼으로도 똑같이 바꾼다
- **기록 탭은 기본 3개월치만 불러온다.** 몇 년 쌓인 뒤에도 탭을 여는 값이 같아야
  하기 때문이다. `그 이전 기록 보기`로 6개월·1년·전체까지 늘린다
...
2. 팀 DB, 개인 DB 각각에서 `... > 연결` 로 이 Integration 추가 (한쪽만 하면 404)
